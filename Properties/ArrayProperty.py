...
class ArrayProperty:
    ...
    def __init__(self, name, binary_read):
        ...
        logger.debug(
            f"array_content_size:{array_content_size} : called from offset:{binary_read.offset}"
        )
        ...
        logger.info(
            f'{text_colours["Magenta"]}ArrayProperty name:{self.name}, subtype:{self.subtype} array_content_size:{array_content_size}, position:{binary_read.offset}'
        )
        ...
        if self.subtype in ["ObjectProperty"]:
            ...
            self.value.append(
                {
                    "type": self.subtype,
                    "value": binary_read.read_bytes(array_content_size),
                }
            )
            ...
            for i in range(int_content_count):
                self.value.append(
                    {"type": self.subtype, "value": binary_read.read_int32()}
                )
            logger.debug(
                f"Array IntProperty >> name:{self.name}, type:{self.type}, subtype:{self.subtype}, value:{self.value}"
            )
        else:
