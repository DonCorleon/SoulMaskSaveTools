...
class StructProperty:
    ...
    def __init__(self, name, binary_read, in_array=False):
        logger.debug(f'{self.__class__.__name__}.{inspect.currentframe().f_code.co_name}')
        if in_array:
            logger.debug(f'{text_colours["Lime Green"]}{binary_read.offset}:Struct is part of array')
        self.type = "StructProperty"
        self.name = name
        struct_start_position = binary_read.offset
        self.content_size = binary_read.read_uint32()
        ...
        logger.debug(f'{text_colours["Green"]}StructProperty name:{self.name}, type:{self.type}, in_array:{in_array}, content_size:{self.content_size}, position:{binary_read.offset}')
        ...
        if self.subtype == "Guid":
            ...
            self.value = StructProperty(self.subname, binary_read)
