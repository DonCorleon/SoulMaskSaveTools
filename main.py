...
class db_read:
    ...
    def get_entries(self, table):
        ...
        with self.connection as conn:
            ...
            logger.debug(f'Table has {row_count} rows and {len(column_names)} columns {column_names}')
    ...
    def decompress_all(self, object, label):
        ...
        logger.info(f'inflated database info from {compressed_size/(1024 * 1024)} MB to {decompressed_size/(1024 * 1024)} MB')
...
class binary_read:
    ...
    def expect_NoneType(self):
        ...
        try:
            ...
            if result == 'None':
                ...
                logger.warning(f'Unexpected data found: {self.read_bytes(20).hex(" ",1)}')
                return False
        except Exception as e:
            logger.error(f'{e}')
    ...
    def expect(self, expected_data):
        ...
        try:
            ...
            if result == expected_data:
                ...
                self.offset = position
                logger.warning(f'Unexpected data found. Looking for {expected_data}, found: {self.read_bytes(20).hex(" ",1)}')
                return False
        except Exception as e:
            logger.error(f'{e}')
    '''
    ...
    def has_finished(self):
        ...
        if self.offset > self.file_size:
            logger.error(f'Overread the buffer, size:{self.file_size}, offset:{self.offset}')
    ...
    def dump_to_disk(self, data, file_path='.'):
        ...
        if os.path.exists(f'{file_path}/previous_object.bin'):
            os.replace(f'{file_path}/previous_object.bin', f'{file_path}/back2_object.bin')  # Overwrites new_filename if it exists
        if os.path.exists(filename):
            os.replace(filename, f'{file_path}/previous_object.bin')  # Overwrites new_filename if it exists
